...
impl Command for TxnRollCommand<'_> {
    ...
    async fn prepare_buffer(&mut self, conn: &mut Connection) -> Result<()> {
        conn.buffer
            .set_txn_roll(self.single_command.key, &self.txn, self.txn_attr)
    }
    ...
    async fn parse_result(&mut self, conn: &mut Connection) -> Result<()> {
        ...
        // KeyNotFoundError is treated as success because the write may have
        // already been rolled (or never landed) and the server has cleaned up.
        match result_code {
