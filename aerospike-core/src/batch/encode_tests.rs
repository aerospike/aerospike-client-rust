...
/// Rows that differ in their bin list cannot repeat, so every row carries a
/// full header. If the encoder ever repeated these, the bins of the first row
/// would silently be applied to all of them.
#[test]
...
/// sequence must therefore repeat nothing, even though rows 0 and 2 are
/// identical.
#[test]
...
/// the previous row" and "a fresh random shape", so repeats and header breaks
/// land at random positions. The oracle is independent of the encoder's
/// bookkeeping: the whole batch must cost exactly the empty-message overhead
/// plus, per row, either one repeat row or that row's own full cost measured
/// by encoding it alone. If the cached repeat decision were ever computed
/// against the wrong predecessor, or the write loop disagreed with the sizing
...
fn random_repeat_sequences_encode_to_the_predicted_size() {
    ...
    for round in 0..300 {
        ...
        for i in 0..n {
            // No transaction here, so the version half of the header is None.
            let repeats = i > 0 && rows[i].0.match_header(Some(&rows[i - 1].0), None, None);
            if repeats {
                repeats_seen += 1;
        ...
        assert_eq!(actual, expected, "round {round} seed {seed:#x}: shapes {shapes:?} encoded to {actual} bytes, oracle predicts {expected}");
    }
    assert!(repeats_seen > 500 && breaks_seen > 500, "the random mix must exercise both outcomes heavily (repeats {repeats_seen}, breaks {breaks_seen})");
}
