...
pub struct ClusterMetrics {
    ...
    #[cfg_attr(feature = "serialization", serde(rename = "connections_in_use"))]
    pub connections_in_use: u64,

    /// Idle connections sitting in the pools across all nodes, read live when
    /// the snapshot is taken.
    #[cfg_attr(feature = "serialization", serde(rename = "connections_in_pool"))]
    ...
    #[cfg_attr(feature = "serialization", serde(rename = "recover_queue_size"))]
    ...
    #[cfg_attr(feature = "serialization", serde(rename = "nodes_invalid"))]
    pub nodes_invalid: u64,

    /// Number of commands that exhausted their retry budget.
    #[cfg_attr(feature = "serialization", serde(rename = "exceeded_max_retries"))]
    pub exceeded_max_retries: u64,

    /// Number of commands that exceeded their total timeout.
    #[cfg_attr(feature = "serialization", serde(rename = "exceeded_total_timeout"))]
...
mod tests {
    ...
    fn cluster_metrics_serializes_with_expected_layout() {
        ...
        assert_eq!(v["open_connections"], 2);
        assert_eq!(v["connections_in_use"], 1);
        assert_eq!(v["connections_in_pool"], 1);
        assert_eq!(v["recover_queue_size"], 0);
        assert_eq!(v["nodes_invalid"], 3);
        assert_eq!(v["exceeded_max_retries"], 0);
        assert_eq!(v["exceeded_total_timeout"], 0);
        // The per-node entry carries the pool and breaker gauges.
        let node = &v["127.0.0.1:3000"];
        assert_eq!(node["open_connections"], 2);
        assert_eq!(node["connections_in_use"], 1);
        assert_eq!(node["connections_in_pool"], 1);
        assert_eq!(node["connections_recovering"], 0);
        assert_eq!(node["error_rate"], 5);
    }
}
