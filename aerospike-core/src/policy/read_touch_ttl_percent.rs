...
/// `ReadTouchTTLPercent` determines how record TTL (time to live) is affected on reads.
///
/// When enabled, the server can efficiently operate as a read-based LRU cache where the least
/// recently used records are expired. The value is expressed as a percentage of the TTL sent on
/// the most recent write such that a read within this interval of the record’s end of life will
/// generate a touch.
///
