...
use common::{Key, Record, Operation, FieldType, ParticleType, Bin};
...
impl Buffer {
    ...
    pub fn set_write<'a>(&mut self, policy: &WritePolicy, operation: &Operation, key: &Key<'a>, bins: &[&Bin<'a>]) -> AerospikeResult<()> {
        try!(self.begin());
    ...
    pub fn set_exists<'a>(&mut self, policy: &WritePolicy, key: &Key<'a>) -> AerospikeResult<()> {
        ...
        let field_count = try!(self.estimate_key_size(key, false));
        try!(self.size_buffer());
        try!(self.write_header(&policy.base_policy, INFO1_READ|INFO1_NOBINDATA, 0, field_count as usize, 0));
        try!(self.write_key(key, false));
        self.end()
    }
    ...
    pub fn set_read<'a>(&mut self,
        ...
        match bin_names {
            ...
            Some(bin_names) => {
                ...
                try!(self.size_buffer());
                try!(self.write_header(policy, INFO1_READ, 0, field_count, bin_names.len()));
                try!(self.write_key(key, false));
                for bin_name in bin_names {
    ...
    pub fn set_read_header<'a>(&mut self,
                       ...
                       -> AerospikeResult<()> {
        try!(self.begin());
        let fieldCount = try!(self.estimate_key_size(key, false));
        try!(self.estimate_operation_size_for_bin_name(""));
        try!(self.size_buffer());
        try!(self.write_header(policy, INFO1_READ|INFO1_NOBINDATA, 0, fieldCount as usize, 1));
        try!(self.write_key(key, false));
        try!(self.write_operation_for_bin_name("", &operation::READ));
        self.end()
    }

    fn estimate_key_size<'a>(&mut self, key: &Key<'a>, send_key: bool) -> AerospikeResult<usize> {
        ...
        if send_key {
            ...
            self.data_offset += key.user_key.estimate_size() + FIELD_HEADER_SIZE as usize + 1;
            field_count += 1;
        }
    ...
    fn write_key<'a>(&mut self, key: &Key<'a>, send_key: bool) -> AerospikeResult<()> {
        ...
        if send_key {
            try!(self.write_field_value(key.user_key, FieldType::Key));
        }
    ...
    fn write_field_value(&mut self,
        ...
        self.write_bytes(&try!(value.as_bytes()));
    ...
    fn write_operation_for_bin(&mut self,
        ...
        // self.checkServerCompatibility(bin.Value)
        ...
        self.write_bytes(&try!(bin.value.as_bytes()));
