...
fn connect() {
    ...
    let t: i64 = 1;
    let key = Key::new(namespace, set_name, Value::from(t));
    let key = as_key!(namespace, set_name, t);
    let key = as_key!(namespace, set_name, &t);
    let key = as_key!(namespace, set_name, 1);
    let key = as_key!(namespace, set_name, &1);
    let key = as_key!(namespace, set_name, 1i8);
    let key = as_key!(namespace, set_name, &1i8);
    let key = as_key!(namespace, set_name, 1u8);
    let key = as_key!(namespace, set_name, &1u8);
    let key = as_key!(namespace, set_name, 1i16);
    let key = as_key!(namespace, set_name, &1i16);
    let key = as_key!(namespace, set_name, 1u16);
    let key = as_key!(namespace, set_name, &1u16);
    let key = as_key!(namespace, set_name, 1i32);
    let key = as_key!(namespace, set_name, &1i32);
    let key = as_key!(namespace, set_name, 1u32);
    let key = as_key!(namespace, set_name, &1u32);
    let key = as_key!(namespace, set_name, 1i64);
    let key = as_key!(namespace, set_name, &1i64);
    // let key = as_key!(namespace, set_name, 1.0f32);
    // let key = as_key!(namespace, set_name, &1.0f32);
    let key = as_key!(namespace, set_name, 1.0f64);
    let key = as_key!(namespace, set_name, &1.0f64);

    let key = as_key!(namespace, set_name, "haha");
    let key = as_key!(namespace, set_name, "haha".to_string());
    let key = as_key!(namespace, set_name, &"haha".to_string());
