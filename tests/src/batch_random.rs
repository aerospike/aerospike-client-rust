...
// every row back at the index it had on input (CLIENT-5461: in place); on a
// Sequence/PreferRack retry it re-splits the batch onto the next replica.
// These tests draw random batch sizes, op mixes, bin lists (so header repeats
// land at random positions), key routing and unroutable rows, and check the
// properties a caller depends on: every row comes back at its own index with
// the right outcome — under every replica policy, and across a retry that has
// to re-route onto another node.
//
// Each test prints its seed on failure so a case reproduces exactly. They need
// a multi-node cluster to say anything about routing.
...
/// Rows the cluster cannot route are per-row outcomes. They must come back
/// marked at exactly their own index, with every routable row unaffected — and
/// a batch with nothing routable at all fails outright.
#[aerospike_macro::test]
...
/// per-node split and the placement back into input order differ each time;
...
/// re-splitting must be dropped by the time the batch returns.
#[aerospike_macro::test]
async fn arc_node_refcounts_return_to_baseline_after_many_random_batches() {
    ...
    for _ in 0..100 {
        ...
        run(&client, &policy, ops).await.expect("batch");
    }
    let batches: Vec<_> = (0..8).map(|_| make(&mut r)).collect();
    futures::future::join_all(batches.into_iter().map(|(p, o)| {
        let client = &client;
        async move { run(client, &p, o).await }
    }))
    .await
    .into_iter()
    .for_each(|res| {
